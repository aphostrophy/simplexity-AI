...
class ProgressiveDeepeningMinimax:
    ...
    def best_movement(self):
        ...
        self.result_shape = random.choice(
            ...
            if(self.state.board[0, col].shape == ShapeConstant.BLANK):
                if(maximizing):
                    ...
                    if(self.state.players[self.n_player].quota[primary_shape] > 0):
                        ...
                        if key in self.cut_node:
                            possible_moves[col*2] = (-math.inf, -math.inf)
                        else:
                            possible_moves[col *
                                           2] = self.minimax(False, depth-1)
                            if(possible_moves[col*2][0] == -math.inf):
                                self.cut_node[key] = True
                    ...
                    if(self.state.players[self.n_player].quota[secondary_shape] > 0):
                        ...
                        if key in self.cut_node:
                            possible_moves[col*2+1] = (-math.inf, -math.inf)
                        else:
                            possible_moves[col*2 +
                                           1] = self.minimax(False, depth-1)
                            if(possible_moves[col*2+1][0] == -math.inf):
                                self.cut_node[key] = True
